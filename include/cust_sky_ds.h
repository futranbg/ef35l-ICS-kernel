...
/*
 - 사용하지 않는 3GPP2 프로파일 프로세스 제거
*/
#define FEATURE_SKY_DS_SKIP_3GPP2_PROFILE_PROCESS

/*
- rild의 예기치 않는 종료 및 재 실행시 data 연결 되지 않는 문제 발생
- CS service가 detach attach되는 과정은 있지만, Data service는 계속 연결된 상태를 유지하고 있음.
- CS service와 service state를 sync시킴. CS only인 경우는 제외함.
 - GsmServiceStateTracker.java
*/
#define FEATURE_SKY_DS_SYNC_CS_SERVICE_STATE

/*
 - system busy 일때 간헐적으로 Data Registration State query 실패하여 data service state 가 out_of_service, radio tech 가 unknown 으로 표시되는 문제
 - ServiceStateTracker 에서 newSS 로 현재 SS 를 update 시키고 clear 시킨 후 다음 polling 시 newSS 값으로 다시 SS 를 업데이트 시키게 되는데 
    이때 newSS clear 시킨 후 data 관련 값은 현재 ss 값을 set 해 주어 error 발생시 그 값이 유지되도록 보완코드 처리함.
*/
...
/*
- WIFI Connected상태에서 3G/LTE Data가 
접속되어도 3G/LTE Icon이 Display 되지 않아 수정.
...
/*
 - Wifi off로서 data붙을 때, wifi interface name을 갖고 오지 못해서 wifi connection이 정리되지 않는 문제
 - Current interface name이 있는 경우에 정리하도록 수정
*/
...
/*
 ...
 - 2012.07.21 SKT요청으로 33S ICS버전부터 HD영상통화 피처 삭제
*/
